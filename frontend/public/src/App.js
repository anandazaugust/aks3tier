...
const App = () => {
  ...
  const handleAddUser = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://74.177.193.39/api/users', { name, email });
  ...
  const handleUpdateUser = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://74.177.193.39/api/users/${updateUserId}`, { name: updateName, email: updateEmail });
  ...
  const handleDeleteUser = async (id) => {
    try {
      await axios.delete(`http://74.177.193.39/api/users/${id}`);
